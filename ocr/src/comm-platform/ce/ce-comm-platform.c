    ...
    if(BLOCK_FROM_ID(self->location) != BLOCK_FROM_ID(target)) {
        ...
        // - Check remote stage Empty/Busy/Full is Empty.
        {
            ...
            if(t == FSIM_COMM_CLEANUP_BUFFER) {
                ...
                RESULT_ASSERT(hal_swap64(&(rq->status), FSIM_COMM_FREE_BUFFER), ==, FSIM_COMM_FREE_BUFFER);
            } else {
        ...
        DPRINTF(DEBUG_LVL_VERB, "PD is at %p, func is at %p\n", self->pd, &(self->pd->fcts.pdMalloc));
        ...
        DPRINTF(DEBUG_LVL_VVERB, "Creating send buffer @ %p of size 0x%"PRIu64"\n",
...
static u8 extractXEMessage(ocrCommPlatformCe_t *cp, ocrPolicyMsg_t **msg, u32 queueIdx) {
    ...
    *msg = cp->base.pd->fcts.pdMalloc(cp->base.pd, cp->lq[queueIdx]->size);
    DPRINTF(DEBUG_LVL_VERB, "Created local buffer @ %p of size %"PRIu64" to copy from %p\n",
        *msg, cp->lq[queueIdx]->size, remoteMsg);
