...
#define PD_TYPE PD_MSG_RESILIENCY_NOTIFY
        ...
        case OCR_FAULT_DATABLOCK_CORRUPTION_XE:
            {
                ocrFatGuid_t fGuid = rself->faultArgs.OCR_FAULT_ARG_FIELD(OCR_FAULT_DATABLOCK_CORRUPTION_XE).db;
                ASSERT(!ocrGuidIsNull(fGuid.guid) && fGuid.metaDataPtr != NULL);
                ocrDataBlock_t *db= (ocrDataBlock_t *)fGuid.metaDataPtr;
                ASSERT(ocrGuidIsEq(fGuid.guid, db->guid));
                DPRINTF(DEBUG_LVL_WARN, "Injecting fault - corrupting data-block "GUIDF" by changing a value to 0xff...f\n", GUIDA(fGuid.guid));
                void *dataPtr = db->ptr;
                *((unsigned long *)dataPtr) = 0xfffffffful;
                break;
            }
        default:
            {
        ...
        PD_MSG_FIELD_O(returnDetail) = retCode;
        break;
#undef PD_MSG
#undef PD_TYPE
    }
...
#define PD_TYPE PD_MSG_RESILIENCY_MONITOR
        ...
        if (rself->faultArgs.kind != OCR_FAULT_NONE) {
            ...
            case OCR_FAULT_DATABLOCK_CORRUPTION_XE:
                {
                    rself->faultArgs.OCR_FAULT_ARG_FIELD(OCR_FAULT_DATABLOCK_CORRUPTION_XE).db.guid = NULL_GUID;
                    rself->faultArgs.OCR_FAULT_ARG_FIELD(OCR_FAULT_DATABLOCK_CORRUPTION_XE).db.metaDataPtr = NULL;
                    break;
                }
            default:
                {
